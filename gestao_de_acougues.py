...
# =========================================================================
# FUNÇÃO AUXILIAR PARA CONVERSÃO SEGURA DE NÚMEROS (PREVINE TYPEERROR E SUPORTA %)
# =========================================================================
...
def init_db():
    ...
    if cursor.fetchone()[0] == 0:
        ins_ali_default = json.dumps([
            {"cod": "001", "produto": "COSTELA", "qtd_bruta": 21.0, "unidade": "KG", "preco_bruto": 24.90, "rendimento_pct": 100.0},
            {"cod": "002", "produto": "PAPRICA DEFUMADA", "qtd_bruta": 0.2, "unidade": "KG", "preco_bruto": 29.00, "rendimento_pct": 100.0},
            {"cod": "003", "produto": "SAL GROSSO", "qtd_bruta": 0.3, "unidade": "KG", "preco_bruto": 6.00, "rendimento_pct": 100.0},
            {"cod": "004", "produto": "AMACIANTE DE CARNES", "qtd_bruta": 0.4, "unidade": "KG", "preco_bruto": 19.00, "rendimento_pct": 100.0}
        ])
        
        ins_nao_ali_default = json.dumps([
            {"cod": "101", "produto": "GAS", "qtd_bruta": 0.25, "unidade": "UNID", "preco_bruto": 130.00, "rendimento_pct": 100.0},
            {"cod": "102", "produto": "EMBALAGEM", "qtd_bruta": 1.0, "unidade": "UNID", "preco_bruto": 70.00, "rendimento_pct": 100.0}
        ])
...
def render_modulo_financeiro():
    ...
    st.markdown("Calcule qualquer parâmetro do empréstimo e gere a tabela de amortização com taxas equivalentes.")
    ...
    with st.form("form_calculo_financeiro"):
        ...
        with col_a:
            pv_input = st.number_input("Valor Financiado / Empréstimo - PV (R$)", min_value=0.0, value=100000.0, step=1000.0) if calculo_opcao != "Valor Financiado (PV)" else 0.0
            pmt_input = st.number_input("Valor da Prestação - PMR / PMT (R$)", min_value=0.0, value=9168.0, step=100.0) if calculo_opcao != "Prestação (PMT)" else 0.0

        with col_b:
            taxa_val = st.number_input("Taxa de Juros (%)", min_value=0.0, value=1.5, step=0.1) if calculo_opcao != "Taxa de Juros (i)" else 0.0
            taxa_unid = st.selectbox("Periodicidade Taxa", ["% a.m.", "% a.a.", "% a.d."]) if calculo_opcao != "Taxa de Juros (i)" else "% a.m."
            prazo_val = st.number_input("Prazo Total", min_value=1, value=12, step=1) if calculo_opcao != "Prazo (n)" else 12
            prazo_unid = st.selectbox("Unidade do Tempo", ["Meses", "Anos", "Dias"]) if calculo_opcao != "Prazo (n)" else "Meses"

        btn_calcular_fin = st.form_submit_button("⚡ Processar Cálculo Financeiro")

    if btn_calcular_fin:
        def converter_para_taxa_mensal(val, unid):
            i_r = val / 100.0
            if unid == "% a.m.": return i_r
            elif unid == "% a.a.": return ((1.0 + i_r) ** (1.0 / 12.0)) - 1.0
            else: return ((1.0 + i_r) ** 30.0) - 1.0

        def converter_para_meses(val, unid):
            if unid == "Meses": return float(val)
            elif unid == "Anos": return float(val * 12)
            else: return float(val) / 30.0
        ...
        try:
            if calculo_opcao == "Prestação (PMT)":
                ...
                pmt_calc = pv_calc * (i_m_calc * (1 + i_m_calc)**n_calc) / ((1 + i_m_calc)**n_calc - 1) if "Price" in sistema and i_m_calc > 0 else (pv_calc / n_calc) + (pv_calc * i_m_calc)

            elif calculo_opcao == "Valor Financiado (PV)":
                pmt_calc = pmt_input
                i_m_calc = converter_para_taxa_mensal(taxa_val, taxa_unid)
                n_calc = int(round(converter_para_meses(prazo_val, prazo_unid)))
                pv_calc = pmt_calc * (((1 + i_m_calc)**n_calc - 1) / (i_m_calc * (1 + i_m_calc)**n_calc)) if "Price" in sistema and i_m_calc > 0 else (pmt_calc * n_calc) / (1 + i_m_calc * (n_calc + 1) / 2.0)

            elif calculo_opcao == "Prazo (n)":
                pv_calc, pmt_calc = pv_input, pmt_input
                i_m_calc = converter_para_taxa_mensal(taxa_val, taxa_unid)
                if pmt_calc <= pv_calc * i_m_calc:
                    st.error("A prestação é menor ou igual aos juros. Empréstimo perpétuo!")
                    return
                n_calc = max(1, int(round(math.log(pmt_calc / (pmt_calc - i_m_calc * pv_calc)) / math.log(1 + i_m_calc))))
            ...
            for mes in range(1, n_calc + 1):
                ...
                amortizacao = (pmt_calc - juros) if "Price" in sistema else (pv_calc / n_calc)
                pmt_efetivo = pmt_calc if "Price" in sistema else (amortizacao + juros)
                saldo_devedor -= amortizacao
                dados.append({"Mês": mes, "Prestação": pmt_efetivo, "Juros": juros, "Amortização": amortizacao, "Saldo Devedor": max(0.0, saldo_devedor)})
            ...
            pdf_bytes_fin = gerar_pdf_relatorio_financeiro(
                pv_calc, i_m_calc, n_calc, sistema, df_amort, df_amort["Prestação"].sum(), df_amort["Juros"].sum(), 
...
def render_modulo_ficha_tecnica():
    ...
    if col_search2.button("📥 Carregar Ficha"):
        if ficha_selecionada == "➕ Criar Nova Ficha Técnica":
            ...
            try:
                raw_ali = json.loads(row_ft['insumos_ali_json']) if row_ft['insumos_ali_json'] else []
                for item in raw_ali:
                    if "rendimento_pct" not in item:
                        item["rendimento_pct"] = safe_float(item.get("rendimento", 1.0)) * 100.0
                st.session_state.ft_items_ali = raw_ali
            except Exception:
                st.session_state.ft_items_ali = []

            try:
                raw_nao_ali = json.loads(row_ft['insumos_nao_ali_json']) if row_ft['insumos_nao_ali_json'] else []
                for item in raw_nao_ali:
                    if "rendimento_pct" not in item:
                        item["rendimento_pct"] = safe_float(item.get("rendimento", 1.0)) * 100.0
                st.session_state.ft_items_nao_ali = raw_nao_ali
            except Exception:
                st.session_state.ft_items_nao_ali = []
    ...
    with tab_ft:
        ...
        st.markdown("##### 📊 Tabela Resumo de Rendimentos Calculados:")
        df_rend_tabela = pd.DataFrame([
            {"Campo da Tabela Rendimentos": "Referência", "Valor Calculado / Entrada": ft_ref},
            {"Campo da Tabela Rendimentos": "Produto", "Valor Calculado / Entrada": ft_produto.upper()},
            {"Campo da Tabela Rendimentos": "Rendimento kg (Crua)", "Valor Calculado / Entrada": f"{ft_rend_crua:.3f} kg"},
            {"Campo da Tabela Rendimentos": "Rendimento Depois de Assada kg", "Valor Calculado / Entrada": f"{ft_rend_assada:.3f} kg"},
            {"Campo da Tabela Rendimentos": "Peso da Unidade KG", "Valor Calculado / Entrada": f"{ft_peso_unid:.3f} kg"},
            {"Campo da Tabela Rendimentos": "Unidades Produzidas [Fórmula: Piso(Assada/Unidade)]", "Valor Calculado / Entrada": f"{int(ft_unid_prod)} un"},
            {"Campo da Tabela Rendimentos": "Quantidade no Pacote", "Valor Calculado / Entrada": f"{int(ft_qtd_pacote)} un/pacote"},
            {"Campo da Tabela Rendimentos": "Perda % [Fórmula: 1 - (Assada/Crua)]", "Valor Calculado / Entrada": f"{perda_pct:.4f}%"}
        ])
        st.dataframe(df_rend_tabela, use_container_width=True, hide_index=True)

        st.markdown("---")
        st.subheader("1. Insumos Alimentícios (Edição Interativa)")
        st.caption("✏️ Edite os insumos diretamente na tabela. A coluna de Rendimento deve ser informada em % (Ex: 100.00 = 100%).")

        df_ali_raw = pd.DataFrame(st.session_state.ft_items_ali)
        if df_ali_raw.empty or "rendimento_pct" not in df_ali_raw.columns:
            if not df_ali_raw.empty and "rendimento" in df_ali_raw.columns:
                df_ali_raw["rendimento_pct"] = df_ali_raw["rendimento"].apply(lambda x: safe_float(x) * 100.0 if safe_float(x) <= 1.0 else safe_float(x))
            else:
                df_ali_raw = pd.DataFrame(columns=["cod", "produto", "qtd_bruta", "unidade", "preco_bruto", "rendimento_pct"])

        df_ali_edit_input = df_ali_raw[["cod", "produto", "qtd_bruta", "unidade", "preco_bruto", "rendimento_pct"]].rename(columns={
            "cod": "Cód",
            "produto": "Produto",
        ...
        edited_ali_df = st.data_editor(
            ...
            column_config={
                ...
                "Rendimento (%)": st.column_config.NumberColumn(format="%.2f %%", min_value=0.0, max_value=500.0)
            }
        )
        ...
        for _, r_ali in edited_ali_df.iterrows():
            ...
            if prod_nome != "":
                ...
                rp = safe_float(r_ali.get("Rendimento (%)"), 100.0)
        ...
        for idx, item in enumerate(st.session_state.ft_items_ali):
            rp = item.get('rendimento_pct', 100.0)
            fator_rend = rp / 100.0
            ql = item['qtd_bruta'] * fator_rend
        ...
        if rows_ali_calc:
            ...
            st.dataframe(df_ali_view.style.format({
                "Qtd Bruta": "{:.3f}", "Preço Bruto (R$)": "R$ {:.2f}",
                "Qtd Líquida": "{:.3f}", "Preço Líquido (R$)": "R$ {:.2f}"
            }), use_container_width=True)

        st.markdown("---")
        st.subheader("2. Insumos Não Alimentícios (Gás, Embalagens, etc. - Edição Interativa)")
        st.caption("✏️ Edite os insumos não alimentícios. A coluna Rendimento é expressa em %.")

        df_nao_ali_raw = pd.DataFrame(st.session_state.ft_items_nao_ali)
        if df_nao_ali_raw.empty or "rendimento_pct" not in df_nao_ali_raw.columns:
            if not df_nao_ali_raw.empty and "rendimento" in df_nao_ali_raw.columns:
                df_nao_ali_raw["rendimento_pct"] = df_nao_ali_raw["rendimento"].apply(lambda x: safe_float(x) * 100.0 if safe_float(x) <= 1.0 else safe_float(x))
            else:
                df_nao_ali_raw = pd.DataFrame(columns=["cod", "produto", "qtd_bruta", "unidade", "preco_bruto", "rendimento_pct"])

        df_nao_ali_edit_input = df_nao_ali_raw[["cod", "produto", "qtd_bruta", "unidade", "preco_bruto", "rendimento_pct"]].rename(columns={
            "cod": "Cód",
            "produto": "Produto",
            "qtd_bruta": "Quantidade Bruta",
            "unidade": "Unidade",
        ...
        for _, r_nao in edited_nao_ali_df.iterrows():
            ...
            if prod_nome != "":
                ...
                rp = safe_float(r_nao.get("Rendimento (%)"), 100.0)
        ...
        tot_ali_custo = sum(item['preco_bruto'] * (item['qtd_bruta'] * (item.get('rendimento_pct', 100.0) / 100.0)) for item in st.session_state.ft_items_ali)
        tot_nao_ali_custo = sum(item['preco_bruto'] * (item['qtd_bruta'] * (item.get('rendimento_pct', 100.0) / 100.0)) for item in st.session_state.ft_items_nao_ali)
    ...
    calc_res_pdf = {
        'tot_ali_custo': tot_ali_custo, 'tot_nao_ali_custo': tot_nao_ali_custo,
...
def render_modulo_ncg():
    ...
    with st.form("form_ncg_completo"):
        ...
        titulo_ncg = st.text_input("Título / Referência da Análise", value=f"Análise NCG {datetime.date.today().strftime('%m/%Y')}")

        btn_calcular_ncg = st.form_submit_button("⚡ Processar Análise de NCG")
    ...
    if st.button("💾 Gravar Análise no Banco de Dados"):
        cursor = conn.cursor()
        data_hoje = str(datetime.date.today())
    ...
    if menu == "Cadastrar Empresa":
        ...
        if menu == "Nova Desossa":
            ...
            if not tipos_empresa:
                ...
                with st.form(f"form_nova_desossa_dados_{v_form}"):
                    col1, col2, col3 = st.columns(3)
                    data_input = col1.date_input("Data da Ação", datetime.date.today(), key=f"date_picker_{v_form}")
                    tipo_animal = col2.selectbox("Tipo de Desossa", tipos_empresa, key=f"tipo_animal_select_{v_form}")
                    peso_bruto = col3.number_input("Peso Bruto (KG)", min_value=0.0, step=0.001, format="%.3f", key=f"input_peso_bruto_{v_form}")
                    
                    col4, col5, col6 = st.columns(3)
                    preco_animal_kg = col4.number_input("Preço do Animal (R$/KG)", min_value=0.0, step=0.01, key=f"input_preco_animal_{v_form}")
                    ossos_muxiba = col5.number_input("Ossos / Muxiba (KG)", min_value=0.0, step=0.001, format="%.3f", key=f"input_ossos_{v_form}")
                    quebra_nao_identificada = col6.number_input("Quebra Não Identificada (KG)", min_value=0.0, step=0.001, format="%.3f", key=f"input_quebra_{v_form}")
                    
                    col7, col8, col9, col10 = st.columns(4)
                    exsudato_escorrimento = col7.number_input("Exsudato / Escorrimento (KG)", min_value=0.0, step=0.001, format="%.3f", key=f"input_exsudato_{v_form}")
                    p_cartao = col8.number_input("Taxas de Cartão (%)", min_value=0.0, max_value=100.0, step=0.01, key=f"input_p_cartao_{v_form}")
                    p_impostos = col9.number_input("Impostos (%)", min_value=0.0, max_value=100.0, step=0.01, key=f"input_p_impostos_{v_form}")
                    p_embalagens = col10.number_input("Embalagens (%)", min_value=0.0, max_value=100.0, step=0.01, key=f"input_p_embalagens_{v_form}")
                    p_comissao = st.number_input("Comissão (%)", min_value=0.0, max_value=100.0, step=0.01, key=f"input_p_comissao_{v_form}")

                    btn_confirmar_lote_dados = st.form_submit_button("⚡ Aplicar Parâmetros da Desossa")
                            ...
                            if st.button("⚡ Importar Cortes do Ficheiro para o Lote", key=f"btn_import_file_{v_form}"):
                                ...
                                for _, r in df_up.iterrows():
                                    st.session_state.cortes_temp.append({
                                        ...
                                        "peso": safe_float(r['peso']),
                                        "preco_venda": safe_float(r['preco_venda'])
                                    })
                ...
                if st.button("💾 Salvar Ação Completa no Banco de Dados", key=f"btn_salvar_db_{v_form}"):
                    if not st.session_state.cortes_temp:
                        ...
                        cortes_tuplas = [(acao_id, c["nome_corte"], c["qualidade"], c["peso"], c["preco_venda"]) for c in st.session_state.cortes_temp]
                        if is_postgres:
                            cursor.executemany("INSERT INTO cortes (acao_id, nome_corte, qualidade, peso, preco_venda) VALUES (%s, %s, %s, %s, %s)", cortes_tuplas)
                        else:
                            cursor.executemany("INSERT INTO cortes (acao_id, nome_corte, qualidade, peso, preco_venda) VALUES (?, ?, ?, ?, ?)", cortes_tuplas)
